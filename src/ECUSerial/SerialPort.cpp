...
bool SerialPort::setBreak() { return SetCommBreak(handler); }
bool SerialPort::clearBreak() { return ClearCommBreak(handler); }
...
SerialPort::SerialPort(const char *portName) noexcept {
    ...
    handler = CreateFileA(portName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
std::optional<uint8_t> SerialPort::fastByteReadUSerialPort() {
    ...
    if (ReadFile(handler, &byte, sizeof(byte), &bytesRead, NULL))
        return std::make_optional<uint8_t>(byte);
...
bool SerialPort::isConnected() const noexcept { return this->connected; }